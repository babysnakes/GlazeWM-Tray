...
// Private routing types — mirrors the private types in MessageParser.fs.
// Redeclared here because F# private types are scoped to their file.
type private CsJsonType =
...
type private CsMessageType =
    | QueryResponseType of string
    | SubscriptionResponseType of string
    | UnSuccessfulResponseType of string

type private CsMessageTypeResult = Result<CsMessageType option, string>

/// A clone of Parser that exposes IObservable streams instead of requiring a
/// MailboxProcessor handler. Intended for use from C# ReactiveUI projects.
type ParserCS() =

    let notifSubject = new Subject<AppNotification>()
    let errorSubject = new Subject<MessageParserEvent>()
    ...
    let reportResultError (sbj: string) (result: Result<_, string>) =
        ...
        |> Result.teeError (fun e ->
            ...
            errorSubject.OnNext(ParseError e))
    ...
    let sendWebSocketMessage (msg: string) =
        ...
        | None ->
            ...
            errorSubject.OnNext UnsetWsClient

    let mergeMatcher (json: string) (f: string -> CsMessageTypeResult) (current: CsMessageTypeResult) : CsMessageTypeResult =
        match current with
        | Ok(Some _) -> current
        | Ok None -> f json
        | Error e -> Error e

    let tryUnsuccessfulResponse (json: string) : CsMessageTypeResult =
        parser {
    ...
    let trySubscriptionEvent (json: string) : CsMessageTypeResult =
        parser {
    ...
    let tryQueryResponse (json: string) : CsMessageTypeResult =
        parser {
    ...
    let handleWorkspacesResponse (wr: WorkspacesResponse) =
        ...
        | Some wn ->
            wn |> Workspaces |> notifSubject.OnNext
            Some(wr)
        | None ->
            Log.Warning("No current workspace found in {Workspaces}", wr.Data)
            errorSubject.OnNext NoCurrentWorkspace
            None
        ...
        parser {
            ...
            if containerType = "window" then
                ...
                return
                    ...
                    |> Option.map (Workspaces >> notifSubject.OnNext)
            else
                return None
        }
        |> Parser.parse json
    ...
    let handleQueryPausedResponse (json: string) =
        parser {
            ...
            return paused |> Paused |> notifSubject.OnNext
        }
        |> Parser.parse json
    ...
    let messageParser =
        let agent =
            MailboxProcessor<CsJsonType>.Start(fun inbox ->
                let rec loop (state: WorkspacesResponse option) =
                    ...
                    async {
                        ...
                        try
                            ...
                            | QueryBindingModes m ->
                                ...
                                result {
                                    ...
                                    return notifSubject.OnNext(NewBindingModes nb)
                                }
                            ...
                            | PausedChanged m ->
                                ...
                                result {
                                    ...
                                    return notifSubject.OnNext(Paused parsed.Data.IsPaused)
                                }
                        ...
                        | ex ->
                            ...
                            errorSubject.OnNext(ParseError ex.Message)
        ...
        agent.Error.Add(fun exn -> errorSubject.OnNext(AgentError exn))
        agent

    member _.Notifications: IObservable<AppNotification> = notifSubject.AsObservable()
    member _.Errors: IObservable<MessageParserEvent> = errorSubject.AsObservable()
                        ...
                        try
                            ...
                            |> function
                                | Ok(Some(UnSuccessfulResponseType msg)) ->
                                    notifSubject.OnNext(UnSuccessfulResponse msg)
                                | Ok(Some(SubscriptionResponseType SFocusCH)) ->
                                ...
                                | Ok(Some(SubscriptionResponseType SPauseCH)) ->
                                    messageParser.Post(PausedChanged msg)
                                | Ok(Some(SubscriptionResponseType SWorkspaceUP))
                                | Ok(Some(SubscriptionResponseType SWorkspaceDeACT))
                                | Ok(Some(SubscriptionResponseType SWorkspaceACT)) ->
                                    messageParser.Post(WorkspaceStar)
                                | Ok(Some(QueryResponseType QWorkspaces)) ->
                                    messageParser.Post(QueryWorkspaces msg)
                                | Ok(Some(QueryResponseType QPaused)) ->
                                    messageParser.Post(QueryPaused msg)
                                | Ok(Some(QueryResponseType QBinding)) ->
                                    messageParser.Post(QueryBindingModes msg)
    ...
    member _.SetWsClient(client: MailboxProcessor<WebSocketMessage>) = wsClient <- Some client

    interface IDisposable with
        member _.Dispose() =
            notifSubject.Dispose()

...
open FSharpReactiveUI.Helpers
open FSharpReactiveUI.Helpers.Notifications
...
module private Assets =
    let loadIcons () : Map<string, WindowIcon> =
...
type App(levelSwitch: LoggingLevelSwitch, logDir: string) as this =
    ...
    let matchStateToIcon (state: TrayIconState) =
        ...
        let name =
            if state.CustomBinding then "qm"
            else state.Workspaces.Current.Name
    ...
    member private _.UpdateTrayMenu(wss: WorkspaceName list) =
        ...
        |> List.iter (fun m ->
            let dn =
                if m.Name = m.DisplayName then $"Workspace {m.Name}"
                else m.DisplayName
    ...
    override _.OnFrameworkInitializationCompleted() =
        ...
        | :? IClassicDesktopStyleApplicationLifetime as desktopLifetime ->
            ...
            let vm = MainWindowViewModel(runSyncQuery)
            let w = MainWindow(DataContext = vm)
            mainWindow <- Some w

            tray.Clicked.Add(fun _ -> toggleMainWindow ())
            tray.Icon <- statusIcons |> Map.find "icon"
